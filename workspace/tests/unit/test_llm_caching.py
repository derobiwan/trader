...
def create_test_snapshot(symbol: str, price: float, rsi: float = 65.0, macd: float = 100.0) -> MarketDataSnapshot:
...
async def test_llm_response_cached():
    ...
    snapshots = {
        'BTC/USDT:USDT': create_test_snapshot('BTC/USDT:USDT', 45000.0, rsi=65.0, macd=100.0)
    }
...
async def test_llm_cache_with_similar_prices():
    ...
    # First call
    signals1 = await engine.generate_signals({'BTC/USDT:USDT': snapshot1}, use_cache=True)
    assert engine._call_llm.call_count == 1

    # Second call with similar price - should hit cache
    signals2 = await engine.generate_signals({'BTC/USDT:USDT': snapshot2}, use_cache=True)
    assert engine._call_llm.call_count == 1  # Not called again
...
async def test_llm_cache_disabled():
    ...
    signals1 = await engine.generate_signals(snapshots, use_cache=False)
    assert engine._call_llm.call_count == 1

    signals2 = await engine.generate_signals(snapshots, use_cache=False)
...
async def test_llm_cache_different_symbols():
    ...
    # Call with BTC
    signals_btc = await engine.generate_signals({'BTC/USDT:USDT': btc_snapshot}, use_cache=True)
    assert signals_btc['BTC/USDT:USDT'].decision == TradingDecision.BUY

    # Call with ETH - should not use BTC cache
    signals_eth = await engine.generate_signals({'ETH/USDT:USDT': eth_snapshot}, use_cache=True)
