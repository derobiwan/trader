...
class PaperTradingPerformanceTracker:
    ...
    def _calculate_metrics(self):
        ...
        if losing_trades:
            ...
            self.metrics['profit_factor'] = gross_profit / gross_loss if gross_loss > 0 else Decimal("0")
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'profitability': {
                ...
                'net_pnl': float(self.metrics['total_pnl'] - self.metrics['total_fees']),
    ...
    def get_symbol_performance(self) -> Dict[str, Dict[str, Any]]:
        ...
        for symbol, stats in symbol_stats.items():
            result[symbol] = {
                ...
                'wins': stats['wins'],
                'losses': stats['losses'],
                'win_rate': float(stats['wins'] / stats['trades'] * 100) if stats['trades'] > 0 else 0.0,
                'total_pnl': float(stats['total_pnl']),
                'total_fees': float(stats['total_fees']),
                'net_pnl': float(stats['total_pnl'] - stats['total_fees']),
                'avg_pnl_per_trade': float(stats['total_pnl'] / stats['trades']) if stats['trades'] > 0 else 0.0,
            }
    ...
    def export_trades_csv(self, filepath: str):
        ...
        with open(filepath, 'w', newline='') as f:
            writer = csv.DictWriter(
                f,
                fieldnames=['timestamp', 'symbol', 'side', 'quantity', 'price', 'fees', 'pnl']
            )
            writer.writeheader()
