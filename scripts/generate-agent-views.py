...
class AgentViewGenerator:
    ...
    def generate_agent_view(self, agent_name):
        ...
        agent_view.append(f"*Generated for minimal context loading*\n")
        
        # Process all PRP documents
        prp_files = []
        for subdir in ['planning', 'architecture', 'contracts', 'security', 'implementation']:
            subdir_path = self.prps_dir / subdir
            if subdir_path.exists():
                prp_files.extend(subdir_path.glob('*.md'))
        
        if not prp_files:
            print(f"⚠️  No PRP documents found")
            # Create a placeholder
            agent_view.append("## No Active Projects\n")
            agent_view.append("No PRP documents available yet. Waiting for project initialization.")
        else:
        ...
        if original_size > 0:
            ...
            print(f"✅ Generated view for {agent_name}: "
                  f"{new_size/1024:.1f}KB (reduced by {reduction:.1f}%)")
        else:
            print(f"✅ Generated placeholder view for {agent_name}: {new_size/1024:.1f}KB")
    ...
    def generate_task_brief(self, task_id):
        ...
        try:
            ...
            print(f"❌ Task registry not found")
            return False
                    ...
                    try:
                        ...
                        if full_path.exists():
                            ...
                            if section_title in content:
                                ...
                                brief.append(f"\n### Referenced: {section}")
                                brief.append(extracted[:500] + "..." if len(extracted) > 500 else extracted)
                    except Exception as e:
        ...
        print(f"✅ Generated task brief for {task_id}: {len(output_content)/1024:.1f}KB")
        return True
    ...
    def generate_all_views(self):
        ...
        if registry_path.exists():
            ...
            active_tasks = [
                task_id for task_id, task in registry.get('tasks', {}).items()
...
def main():
    ...
    parser.add_argument('--clean', action='store_true', help='Clean cache before generating')
