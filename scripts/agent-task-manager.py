...
class AgentTaskManager:
    ...
    def claim_task(self, agent_name, task_id):
        ...
        # Generate session ID
        session_id = (
            f"claude-{datetime.now().strftime('%Y%m%d')}-{uuid.uuid4().hex[:6]}"
        )
        ...
        if task.get("status") == "in-progress":
            print(
                f"⚠️  Warning: Task {task_id} is already in progress by {task.get('agent')}"
            )
            return False
    ...
    def list_tasks(self, status=None, agent=None):
        ...
        for task_id, task in tasks.items():
            ...
            print(
                f"{task_id:<10} {status_emoji} {task.get('status', 'pending'):<10} "
...
def main():
    ...
    handoff_parser = subparsers.add_parser(
        "handoff", help="Handoff task to another agent"
    )
