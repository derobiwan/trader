...
class StoryDev:
    ...
    def list_stories(self, status: str = None):
        ...
        if not stories or (len(stories) == 1 and "STORY-EXAMPLE" in stories):
            ...
            print(
                "Create stories first, then run: python scripts/bmad-integration.py sync-stories"
            )
            return False
        ...
        for story_id, story in sorted(stories.items()):
            ...
            print(
                f"{story_id:<15} {status_emoji} {story.get('status', 'pending'):<10} "
    ...
    def story_status(self, story_id: str):
        ...
        if view_file.exists():
            ...
            print("⚠️  Agent view not generated yet")
    ...
    def complete_story(self, story_id: str, notes: str = ""):
        ...
        cmd = (
            f"python {BASE_DIR}/scripts/agent-task-manager.py complete --task {task_id}"
        )
        if notes:
            cmd += f' --notes "{notes}"'
